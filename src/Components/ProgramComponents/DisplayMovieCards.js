...
const DisplayMovieCards = () => {
  ...
  const routeChange = (id) => {
    if (id === 9999) history.push("./upcoming");
    else {
      let path = `./movie/${id}`;
      history.push(path);
    }
  ...
  if (!ms.Movies || ms.Movies.length === 0) {
    ...
    Movies.forEach((movie) => {
      ...
      if (year > parseInt(thisMovieYear)) displayMovies.push(movie);
      if (parseInt(thisMovieYear) === year)
        if (month > thisMovieMonth) displayMovies.push(movie);
      if (thisMovieMonth === month)
        if (date > thisMovieDate) displayMovies.push(movie);
    });
    return (
      <CardGroup>
        {displayMovies.map((movie, index) => (
          <Card
            className="upcomingCardOnClick"
            key={index}
            border="dark"
            style={{ width: "15rem" }}
          ...
          >
            <Card.Body>
              ...
              <Card.Text>
                <b>Første visning d. {movie.firstFeatureDate}</b>
                <br />
                {movie.description}
              </Card.Text>
            </Card.Body>
